...
int _cmp_docs(const void *a, const void *b) {
  ...
  if (aa->id.size == bb->id.size)
    return memcmp(aa->id.buf, bb->id.buf, aa->id.size);
  else {
...
void print_filesize(char *filename) {
  ...
  printf("file size : %lu bytes (%s)\n", (unsigned long)size,
         print_filesize_approx(size, buf));
}
...
uint64_t print_proc_io_stat(char *buf, int print) {
  ...
  sprintf(buf, "/sys/devices/virtual/misc/ocs/kvdev0/host_write_bytes");
  char str[64];
  int ret;
  (void)ret;
  unsigned long temp;
  ...
  while (!feof(fp)) {
    ...
    if (!strcmp(str, "host_write_bytes:")) {
      val = temp;
      if (print) {
        lprintf("[proc IO] %lu bytes written (%s)\n", (unsigned long)val,
                print_filesize_approx(val, str));
      }
    }
  }
  fclose(fp);
...
int empty_callback(Db *db, DocInfo *docinfo, void *ctx) { return 0; }
...
int get_value_size_by_ratio(struct bench_info *binfo, size_t idx) {
  ...
  if (idx % 100 < binfo->value_size_ratio[0]) {
    value_size = binfo->value_size[0] == 0 ? 4096 : binfo->value_size[0];
             ...
             binfo->value_size_ratio[0] + binfo->value_size_ratio[1]) {
    value_size = binfo->value_size[1] == 0 ? 4096 : binfo->value_size[1];
  } else if (idx % 100 < binfo->value_size_ratio[0] +
                             binfo->value_size_ratio[1] +
                             binfo->value_size_ratio[2]) {
    value_size = binfo->value_size[2] == 0 ? 4096 : binfo->value_size[2];
  } else if (idx % 100 <
             binfo->value_size_ratio[0] + binfo->value_size_ratio[1] +
                 binfo->value_size_ratio[2] + binfo->value_size_ratio[3]) {
...
void _create_doc(struct bench_info *binfo, size_t idx, Doc **pdoc,
                 DocInfo **pinfo, int seq_fill, int socketid, mempool_t *kp,
                 mempool_t *vp) {
  int r, keylen = 0;
  uint32_t crc;
  Doc *doc = *pdoc;
  ...
  if (binfo->bodylen.type == RND_FIXED) {
    ...
    if (doc->data.buf == NULL || binfo->kv_write_mode == 0) {
      ...
      if (doc->data.buf == NULL) {
        fprintf(stderr, "No pool elem available for value - %d\n",
                vp->num_freeblocks);
        exit(1);
      }
      // doc->data.buf = (char*)malloc(4096);
...
void pass_lstat_to_db(Db *db, latency_stat *l_read, latency_stat *l_write,
...
#else
      _create_doc(binfo, i, &docs[i], NULL, binfo->seq_fill, args->socketid,
                  args->keypool, args->valuepool);
#endif
    }
...
// couchstore_free_document(docs[i]);
    ...
    while (c < binfo->ndocs) {
      ...
      while (args->cur_qdepth < binfo->queue_depth) {
        if (c >= binfo->ndocs)
          break;
        _create_doc(binfo, c, &docs[0], NULL, binfo->seq_fill, args->socketid,
                    args->keypool, args->valuepool);
      ...
      if (args->cur_qdepth > 0) {
        ...
        for (j = 0; j < nr; j++) {
          if (contexts[j])
            notify(contexts[j], args->keypool, args->valuepool);
        }
...
void *pop_print_time(void *voidargs) {
  ...
  while (counter < binfo->ndocs * binfo->nfiles) {
    ...
    defined(__FDB_BENCH) || defined(__WT_BENCH) || defined(__LEVEL_BENCH) ||   \
    defined(__KVDB_BENCH)
      ...
      // # inserted documents
      printf("%" _F64 " / %" _F64, counter,
             (uint64_t)binfo->ndocs * binfo->nfiles);
      // throughput (average, instant)
      printf(" (%.2f ops/sec, %.2f ops/sec, ", iops, iops_i);
      // percentage
      printf("%.1f %%, ",
             (double)(counter * 100.0 / (binfo->ndocs * binfo->nfiles)));
      ...
      if (insert_ops_fp) {
        fprintf(insert_ops_fp, "%d.%01d,%.2f,%.2f,%" _F64 ",%" _F64 "\n",
                (int)tv.tv_sec, (int)(tv.tv_usec / 100000), iops, iops_i,
                (uint64_t)counter, bytes_written);
      }
    } else {
...
void _print_percentile(struct bench_info *binfo, struct latency_stat *l_wstat,
                       struct latency_stat *l_rstat,
                       struct latency_stat *l_dstat, const char *unit,
                       int mode);
void population(Db **db, struct bench_info *binfo) {
  ...
  struct pop_thread_args args[binfo->pop_nthreads * binfo->nfiles +
                              1]; // each file/device maps to pop_nthreads
  ...
  for (i = 0; i < binfo->pop_nthreads * binfo->nfiles; ++i) {
    ...
    */
    ...
    fprintf(
        stdout,
        "thread %d initialize key pool - base is %p, next free %p free %d\n",
        (int)i, args[i].keypool->base, args[i].keypool->nextfreeblock,
        args[i].keypool->num_freeblocks);
    fprintf(stdout, "thread %d initialize value pool - base is %p, next free "
                    "%p free %d, unit size %ld\n",
            (int)i, args[i].valuepool->base, args[i].valuepool->nextfreeblock,
            args[i].valuepool->num_freeblocks, info_value.unitsize);

    args[i].l_write =
        (struct latency_stat *)calloc(1, sizeof(struct latency_stat));
    args[i].l_write->samples =
        (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max);
  }
...
// Linux + (LevelDB or RocksDB): wait for background compaction
    ...
    defined(__LEVEL_BENCH) //|| defined(__KVDB_BENCH) //|| defined
  ...
  latency_ms = (long double)totalmicrosecs /
               ((long double)binfo->ndocs * (long double)binfo->nfiles);
  ...
  if (binfo->latency_rate) {
    ...
    l_stat.samples = (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max *
                                        binfo->pop_nthreads * binfo->nfiles);
    tmp = l_stat.samples;
    for (i = 0; i < binfo->pop_nthreads * binfo->nfiles; i++) {
      l_stat.nsamples += args[i].l_write->nsamples;
      memcpy(tmp, args[i].l_write->samples,
             sizeof(uint32_t) * args[i].l_write->nsamples);
...
void _bench_result_init(struct bench_result *result, struct bench_info *binfo) {
  ...
  result->doc_hit = (struct bench_result_hit *)malloc(
      sizeof(struct bench_result_hit) * binfo->ndocs);
  result->file_hit = (struct bench_result_hit *)malloc(
...
void _bench_result_print(struct bench_result *result) {
  ...
  for (i = 0; i < result->nfiles; ++i)
    file_sum += result->file_hit[i].hit;
  for (i = 0; i < result->ndocs; ++i)
    doc_sum += result->doc_hit[i].hit;

  qsort(result->file_hit, result->nfiles, sizeof(struct bench_result_hit),
        _bench_result_cmp);
  qsort(result->doc_hit, result->ndocs, sizeof(struct bench_result_hit),
        _bench_result_cmp);
  ...
  for (i = 0; i < result->nfiles; ++i) {
    ...
    fprintf(fp, "%8d%8d%8.1f %%%8.1f %%\n", (int)result->file_hit[i].idx,
            ...
            100.0 * result->file_hit[i].hit / file_sum, 100.0 * cum / file_sum);
  }
  ...
  for (i = 0; i < result->ndocs; ++i) {
    ...
    if (result->doc_hit[i].hit > 0) {
      fprintf(fp, "%8d%8d%8.1f %%%8.1f %%\n", (int)result->doc_hit[i].idx,
              ...
              100.0 * result->doc_hit[i].hit / doc_sum, 100.0 * cum / doc_sum);
    }
  }
  fclose(fp);
}
  ...
  if (dir_info != NULL) {
    while ((dir_entry = readdir(dir_info))) {
      if (!strncmp(dir_entry->d_name, filename, strlen(filename))) {
        ...
        result = 1;
        if (compaction_no) {
          sscanf(dir_entry->d_name + (filename_len - dirname_len), "%d.%d",
                 &db_no, &cpt_no);
...
int iterate_callback(Db *db, int depth, const DocInfo *doc_info,
...
couchstore_error_t couchstore_open_document_kv(Db *db, sized_buf *key,
                                               sized_buf *value,
                                               couchstore_open_options options);
couchstore_error_t
couchstore_open_document_kvrocks(Db *db, const void *id, size_t idlen,
                                 size_t valuelen, Doc **pDoc,
                                 couchstore_open_options options);
couchstore_error_t
couchstore_delete_document_kv(Db *db, sized_buf *key,
                              couchstore_open_options options);
...
void *bench_thread(void *voidargs) {
  ...
  // uint64_t op_w, op_r, op_d, op_w_cum, op_r_cum, op_d_cum, op_w_turn,
  // op_r_turn, op_d_turn;
  ...
  db_idx = args->id % binfo->nfiles; // for kvssd, # files = # devices, single
                                     // thread pre device
#elif defined __BLOBFS_ROCKS_BENCH
  int singledb_thread_num =
      binfo->nreaders + binfo->niterators + binfo->nwriters + binfo->ndeleters;
  ...
  */
  ...
  while (!args->terminate_signal) {
    ...
    !defined(__KVROCKS_BENCH) && !defined(__AS_BENCH)
#if defined(__BLOBFS_ROCKS_BENCH)
    ...
    if(args->mode > 0){
      ...
      } else if(cur_op_idx % 100 < binfo->ratio[0] + binfo->ratio[1] +
    binfo->ratio[2]){
        write_mode = 2; // read
      } else {
        write_mode = 4; // delete
    ...
    */
    ...
    if (binfo->kv_write_mode == 1) { // sync mode
      ...
      if (args->mode > 0) {
        ...
        if (cur_op_idx % 100 < binfo->ratio[1] + binfo->ratio[2]) {
          write_mode = 1; // write: update/insert
        } else if (cur_op_idx % 100 <
                   binfo->ratio[0] + binfo->ratio[1] + binfo->ratio[2]) {
          write_mode = 2; // read
        } else {
          write_mode = 4; // delete
        }
      ...
      if (write_mode == 1) { // write
        _create_doc(binfo, r, &rq_doc, NULL, binfo->seq_fill, args->socketid,
                    args->keypool, args->valuepool);
...
#if defined __AS_BENCH
        err = couchstore_save_document(NULL, rq_doc, NULL, monitoring);
        DeAllocate(rq_doc->id.buf, args->keypool);
        rq_doc->id.buf = NULL;
#elif defined __KV_BENCH
        err = couchstore_save_document(db[db_idx], rq_doc, NULL, monitoring);
        DeAllocate(rq_doc->id.buf, args->keypool);
        rq_doc->id.buf = NULL;
#else
        err = couchstore_save_document(db[db_idx], rq_doc, NULL,
                                       binfo->compression);
// DeAllocate(rq_doc->id.buf, args->keypool);
        ...
        err = couchstore_open_document(db[db_idx], rq_id.buf, rq_id.size, NULL,
                                       monitoring);
#elif defined __KVROCKS_BENCH
        err = couchstore_open_document_kvrocks(
            db[db_idx], rq_doc->id.buf, rq_doc->id.size, rq_doc->data.size,
            NULL, monitoring);
#else
...
#else
        err = couchstore_delete_document(db[db_idx], rq_doc->id.buf,
                                         rq_doc->id.size, monitoring);
#endif
        DeAllocate(rq_doc->id.buf, args->keypool);
        rq_doc->id.buf = NULL;
      }
      ...
      if (binfo->with_iterator > 0) {
        ...
        if (binfo->with_iterator == 2) { // iterator alone
          ...
          uint64_t gap = t2.tv_sec * 1000000000L + t2.tv_nsec -
...
#endif
      if (args->cur_qdepth < binfo->queue_depth) {
        if (args->terminate_signal)
          break;
#if defined __KV_BENCH
        if (binfo->with_iterator == 1 && iterator_send == 0 &&
            args->cur_qdepth < binfo->queue_depth - 1) {
        ...
        if (write_mode == 1) { // write
...
#if defined __KV_BENCH
          rq_doc->data.buf = (char *)Allocate(
              args->valuepool); //(char*)kvs_zalloc(1024, 4096);
          if (binfo->bodylen.type == RND_RATIO)
            rq_doc->data.size = get_value_size_by_ratio(binfo, r);
          else
            rq_doc->data.size = binfo->bodylen.a; // binfo->vp_unitsize;
          ...
          couchstore_open_document_kv(db[db_idx], &rq_doc->id, &rq_doc->data,
                                      monitoring);
...
// DeAllocate(rq_doc->id.buf, args->keypool);
          ...
          couchstore_open_document(db[db_idx], rq_doc->id.buf, rq_doc->id.size,
                                   NULL, monitoring);
#endif
        } else { // delete
          if (rq_doc == NULL)
          ...
          couchstore_delete_document_kv(db[db_idx], &rq_doc->id, monitoring);
#else
          couchstore_delete_document(db[db_idx], rq_doc->id.buf,
                                     rq_doc->id.size, monitoring);
#endif
        }
        args->cur_qdepth++;
  ...
  args->b_stat->op_read.fetch_add(args->op_read.load(),
                                  std::memory_order_release);
  args->b_stat->op_write.fetch_add(args->op_write.load(),
                                   std::memory_order_release);
  args->b_stat->op_delete.fetch_add(args->op_delete.load(),
                                    std::memory_order_release);
...
couchstore_error_t couchstore_set_compaction(int mode, size_t compact_thres,
                                             size_t block_reuse_thres);
...
couchstore_error_t couchstore_set_page_size(size_t leaf_pg_size,
                                            size_t int_pg_size);
// non-standard functions for aerospike
couchstore_error_t couchstore_as_setup(const char *hosts, uint16_t port,
                                       uint32_t loop_capacity,
                                       const char *name_space, int write_mode);
// non-standard functions for extension for kvs & kvv
couchstore_error_t couchstore_setup_device(const char *dev_path,
                                           char **device_names,
                                           char *config_file, int num_devices,
...
int getevents(Db *db, int min, int max, IoContext **context);
int release_context(Db *db, IoContext **contexts, int nr);
couchstore_error_t couchstore_kvs_set_aio_option(int queue_depth,
                                                 char *core_masks,
                                                 char *cq_thread_masks,
...
void _print_time(struct latency_stat *l_prep, struct latency_stat *l_real) {
  ...
  if (timelog_fp) {
    for (i = 0; i < l_prep->nsamples; i++) {
      fprintf(timelog_fp, "%d %d\n", (int)l_prep->samples[i],
              (int)l_real->samples[i]);
    }
  }
}
#endif

void _print_latency(struct bench_info *binfo, struct latency_stat *l_stat,
                    const char *unit) {
  ...
  lprintf("%d samples (%d Hz), average: %.2f %s\n", (int)l_stat->nsamples,
          (int)binfo->latency_rate, avg, unit);

  for (i = 0; i < 6; ++i) { // screen: only 7 percentiles
    pos = l_stat->nsamples * percentile[i] / 10000;
    if (i == 5 && l_stat->nsamples < 10000) {
      lprintf("N/A %s (%.2f%%)", unit, (double)percentile[i] / 100);
    } else {
      lprintf("%d %s (%.2f%%)", (int)l_stat->samples[pos], unit,
              (double)percentile[i] / 100);
    }
    if (i + 1 < 6) {
      lprintf(", ");
    } else {
...
/*
  ...
  mode = 2: evaluation
 */
void _print_percentile(struct bench_info *binfo, struct latency_stat *wl_stat,
                       struct latency_stat *rl_stat,
                       struct latency_stat *dl_stat, const char *unit,
                       int mode) {
  int percentile[6] = { 100, 500, 5000, 9500, 9900, 9999 };
  ...
  if (tmp) { // log file: all percentiles
    ...
    for (i = 1; i < 100; ++i) {
      ...
      if (wl_stat && rl_stat) {
        ...
        fprintf(tmp, "%d,%d,%d\n", (int)i,
    (int)wl_stat->samples[pos1],(int)rl_stat->samples[pos2]);
      }
      else if(wl_stat) {
        pos1 = wl_stat->nsamples * i / 100;
...
#if !defined(__COUCH_BENCH) && !defined(__KV_BENCH) &&                         \
    !defined(__KVROCKS_BENCH) && !defined(__AS_BENCH)
...
#if defined(__ROCKS_BENCH) || defined(__BLOBFS_ROCKS_BENCH) ||                 \
    defined(__LEVEL_BENCH) || defined(__KVDB_BENCH)
// RocksDB: set compaction style
  ...
  couchstore_kvs_set_aio_option(binfo->queue_depth, binfo->core_ids,
                                binfo->cq_thread_ids, binfo->mem_size_mb);
  couchstore_kvs_set_aiothreads(binfo->aiothreads_per_device);
  couchstore_kvs_set_coremask(binfo->core_ids);
  //}
  couchstore_setup_device(binfo->kv_device_path, NULL,
                          binfo->kv_emul_configfile, binfo->nfiles,
                          binfo->kv_write_mode, 0 /*binfo->is_polling*/);
#endif
#if defined(__AS_BENCH)
  // TODO: other initialization for aerospike
  couchstore_as_setup(binfo->device_path, binfo->as_port, binfo->loop_capacity,
                      binfo->name_space, binfo->kv_write_mode);
#endif
#if defined(__BLOBFS_ROCKS_BENCH)
...
void do_bench(struct bench_info *binfo) {
  ...
  uint64_t op_count_read, op_count_write, op_count_delete, op_count_iter_key,
      display_tick = 0;
...
// struct latency_stat l_read, l_write, l_delete;
  ...
  // couchstore_setup_device(binfo->kv_device_path, NULL, NULL, binfo->nfiles,
  // binfo->kv_write_mode);
  char *pt;
  i = 0;
...
// === initialize and populate files ========
    ...
    defined(__BLOBFS_ROCKS_BENCH) || defined(__KVDB_BENCH)
...
#endif
#if defined(__LEVEL_BENCH) || defined(__ROCKS_BENCH) ||  \
    defined(__KVROCKS_BENCH) || defined(__BLOBFS_ROCKS_BENCH) ||               \
    defined(__KVDB_BENCH)
        if (!binfo->pop_commit) {
...
// TODO: check whether need to open aerospike connection
    ...
    couchstore_open_conn((char *)binfo->filename);
#elif defined(__ROCKS_BENCH) || defined(__FDB_BENCH) ||                        \
    defined(__LEVEL_BENCH) || defined(__KVDB_BENCH)
...
#if defined(__ROCKS_BENCH) || defined(__LEVEL_BENCH) ||                        \
    defined(__KVDB_BENCH) // || defined(__BLOBFS_ROCKS_BENCH)
  ...
  if (binfo->batch_dist.type == RND_ZIPFIAN) {
    ...
    if (binfo->nops > 0) {
      zipf_rnd_init(&zipf, (binfo->ndocs + binfo->nops * binfo->ratio[2] * 2) /
                               binfo->batch_dist.b,
                    binfo->batch_dist.a / 100.0, 1024 * 1024);
    } else {
      zipf_rnd_init(&zipf,
                    binfo->ndocs * binfo->amp_factor / binfo->batch_dist.b,
                    binfo->batch_dist.a / 100.0, 1024 * 1024);
    }
  }
  ...
  // thread args
  /*
  if (binfo->nreaders + binfo->niterators + binfo->nwriters + binfo->ndeleters
  == 0){
  ...
  int db_idx, td_idx, nodeid;
  int total_ratio =
      binfo->ratio[0] + binfo->ratio[1] + binfo->ratio[2] + binfo->ratio[3];
  pool_info_t info_key, info_value;
  // initialize keypool & value pool
  info_key.allocatortype = info_value.allocatortype = binfo->allocatortype;
  info_key.numunits = binfo->kp_numunits;
  ...
  for (i = 0; i < bench_threads; ++i) {
    if (total_ratio > 0 && total_ratio <= 100) {
      ...
      } else if ((size_t)i % singledb_thread_num <
                 binfo->nwriters + binfo->nreaders) {
        b_args[i].mode = 2; // reader
      } else if ((size_t)i % singledb_thread_num <
                 binfo->nwriters + binfo->nreaders + binfo->niterators) {
    ...
    b_args[i].l_delete =
        (struct latency_stat *)calloc(1, sizeof(struct latency_stat));
    b_args[i].l_read->samples =
        (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max);
    b_args[i].l_write->samples =
        (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max);
    b_args[i].l_delete->samples =
        (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max);
    b_args[i].op_read = b_args[i].op_write = b_args[i].op_delete =
        b_args[i].op_iter_key = 0;
...
// open db instances
...
#elif defined(__ROCKS_BENCH) || defined(__LEVEL_BENCH) ||                      \
    defined(__KVDB_BENCH) //|| defined(__BLOBFS_ROCKS_BENCH)
    ...
    if(i % singledb_thread_num == 0) {
      b_args[i].db = (Db**)malloc(sizeof(Db*));
      couchstore_open_db_kvs(dev_path[i / singledb_thread_num],
    &b_args[i].db[0], i / singledb_thread_num);
    } else {
      first_db_idx = i / singledb_thread_num * singledb_thread_num;
      b_args[i].db = b_args[first_db_idx].db;
      }
...
// b_args[i].blobfs_env = blobfs_env;
  ...
  for (i = 0; i < bench_threads; ++i) {
    pthread_create(&bench_worker[i], &attr[i], bench_thread,
                   (void *)&b_args[i]);
  }
...
// TBD: get KVS bytes written
        ...
        fprintf(tmp, "%d.%01d,%.2f,%.2f,"
                ...
                (double)((op_count_read + op_count_write + op_count_delete) -
                         (prev_op_count_read + prev_op_count_write +
                          prev_op_count_delete)) /
                    (_gap.tv_sec + (double)_gap.tv_usec / 1000000.0),
                op_count_read, op_count_write, written_final - written_init);
      }
      ...
      if (display_tick % filesize_chk_term == 0) {
        ...
        if (binfo->auto_compaction) { // auto
          ...
          sprintf(curfile_temp, "%s%d.%d", binfo->filename, (int)curfile_no,
      ...
      printf("%s/s ", print_filesize_approx(
                          (written_final - written_init) / elapsed_time, cmd));
      ...
      printf("%s/s ", print_filesize_approx(
                          (written_final - written_prev) /
                              (print_term_ms * filesize_chk_term / 1000.0),
                          cmd));
      // instant write amplification
      inst_written_doc =
          (op_count_write - prev_op_count_write) * filesize_chk_term;
      if (inst_written_doc) {
      ...
      if ((size_t)sw.elapsed.tv_sec >= binfo->warmup_secs) {
        if (warmingup) {
          ...
          */
          for (j = 0; j < bench_threads; j++) {
            b_args[j].op_read = b_args[j].op_write = b_args[j].op_delete =
                b_args[j].op_iter_key = 0;
...
// each thread will check outstanding IOs individually
  ...
  if (op_count_delete) {
    lprintf("%" _F64 " deletes (%.2f ops/sec, %.2f us/delete)\n",
            op_count_delete, (double)op_count_delete / gap_double,
  ...
  if (!binfo->auto_compaction) {
    // manual compaction
    lprintf("compaction : occurred %d time%s, ", total_compaction,
...
#if defined(__PRINT_IOSTAT)
  {
    ...
    if (op_count_write) {
      ...
      lprintf("average disk write throughput: %.2f MB/s\n",
              (double)written / (gap.tv_sec * 1000000 + gap.tv_usec) * 1000000 /
                  (1024 * 1024));
      waf_a = (double)w_per_doc / avg_docsize;
      lprintf("%s written per doc update (%.1f x write amplification)\n",
              print_filesize_approx(w_per_doc, bodybuf), waf_a);
  ...
  if (binfo->latency_rate && binfo->with_iterator != 2) {
    ...
    w_stat.samples = (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max *
                                        bench_threads);
    r_stat.samples = (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max *
                                        bench_threads);
    d_stat.samples = (uint32_t *)malloc(sizeof(uint32_t) * binfo->latency_max *
                                        bench_threads);
    t1 = w_stat.samples;
    t2 = r_stat.samples;
    t3 = d_stat.samples;
    ...
    for (i = 0; i < bench_threads; i++) {
      ...
      memcpy(t1, b_args[i].l_write->samples,
             sizeof(uint32_t) * b_args[i].l_write->nsamples);
      memcpy(t2, b_args[i].l_read->samples,
             sizeof(uint32_t) * b_args[i].l_read->nsamples);
      memcpy(t3, b_args[i].l_delete->samples,
             sizeof(uint32_t) * b_args[i].l_delete->nsamples);
...
void _print_benchinfo(struct bench_info *binfo) {
  ...
  if (binfo->keyfile) {
    ...
    lprintf("key data: %s (avg length: %d)\n", binfo->keyfile,
            (int)binfo->avg_keylen);
  } else {
    if (binfo->keylen.type == RND_FIXED) {
      lprintf("key length: %s(%d) / ", "Fixed size", (int)binfo->keylen.a);
    } else {
      // random keygen
  ...
  if (binfo->bodylen.type == RND_FIXED) {
    lprintf("body length: %s(%d)\n", "Fixed size", (int)binfo->bodylen.a);
  } else {
  ...
  lprintf(" (-%d ~ +%d, total %d)\n", (int)binfo->batchrange,
          (int)binfo->batchrange, (int)binfo->batchrange * 2);
  // if (binfo->write_prob <= 100) {
  if (binfo->ratio[1] + binfo->ratio[2] <= 100) {
...
void init_cpu_aff(struct bench_info *binfo) {
  ...
  int total_bench_threads =
      binfo->nreaders + binfo->niterators + binfo->nwriters + binfo->ndeleters;
  binfo->instances =
      (instance_info_t *)malloc(sizeof(instance_info_t) * binfo->nfiles);
  for (i = 0; i < binfo->nfiles; i++) {
    binfo->instances[i].coreids_pop =
        (int *)malloc(sizeof(int) * binfo->pop_nthreads);
    for (j = 0; j < binfo->pop_nthreads; j++)
      binfo->instances[i].coreids_pop[j] = -1;

    binfo->instances[i].coreids_bench =
        (int *)malloc(sizeof(int) * total_bench_threads);
  ...
  binfo->cpuinfo->cpulist =
      (int **)calloc(1, sizeof(int *) * binfo->cpuinfo->num_numanodes);
  for (i = 0; i <= binfo->cpuinfo->num_numanodes; i++) {
    binfo->cpuinfo->cpulist[i] =
        (int *)calloc(1, sizeof(int) * binfo->cpuinfo->num_cores_per_numanodes);
  }
  ...
  while (fgets(line, sizeof line, fp) != NULL) {
    ...
    binfo->cpuinfo->cpulist
        [nodeid][core_count++ % binfo->cpuinfo->num_cores_per_numanodes] =
        coreid;
  ...
  for (i = 0; i < binfo->nfiles; i++) {
    printf("pop -- dev %s, numaid %d, core: ", binfo->device_name[i],
           binfo->instances[i].nodeid_load);
    for (j = 0; j < binfo->pop_nthreads; j++)
      printf("%d ", binfo->instances[i].coreids_pop[j]);
    printf("\n");
  }
  for (i = 0; i < binfo->nfiles; i++) {
    printf("bench --- dev %s, numaid %d, core: ", binfo->device_name[i],
           binfo->instances[i].nodeid_perf);
    for (j = 0; j < total_bench_threads; j++)
...
struct bench_info get_benchinfo(char *bench_config_filename, int config_only) {
  ...
  if (config_only) {
    ...
    printf("total %d nodes, %d cores per node\n", cpuinfo->num_numanodes,
           cpuinfo->num_cores_per_numanodes);
    free(cpuinfo);
    exit(0);
  } else {
...
#elif __ROCKS_BENCH
  sprintf(dbname, "ShannonDB");
  sprintf(dbname_postfix, "shannon");
  ...
  binfo.kp_numunits =
      iniparser_getint(cfg, (char *)"system:key_pool_size", 128);
  binfo.kp_unitsize = iniparser_getint(cfg, (char *)"system:key_pool_unit", 16);
  binfo.kp_alignment =
      iniparser_getint(cfg, (char *)"system:key_pool_alignment", 4096);
  binfo.vp_numunits =
      iniparser_getint(cfg, (char *)"system:value_pool_size", 128);
  binfo.vp_unitsize =
      iniparser_getint(cfg, (char *)"system:value_pool_unit", 4096);
  binfo.vp_alignment =
      iniparser_getint(cfg, (char *)"system:value_pool_alignment", 4096);
  ...
  if (str[0] == 'i' || str[0] == 'I')
    binfo.store_option = 0; // KVS_STORE_IDEMPOTENT
  else if (str[0] == 'p' || str[0] == 'P')
    binfo.store_option = 1; // KVS_STORE_POST
  else
    binfo.store_option = 2; // KVS_STORE_APPEND

  binfo.queue_depth = iniparser_getint(cfg, (char *)"kvs:queue_depth", 8);
  if (binfo.kv_write_mode == 0) { // aio
    if (binfo.queue_depth > binfo.kp_numunits ||
        binfo.queue_depth > binfo.vp_numunits) {
      fprintf(stderr, "wARN: Please set key/value pool unit equal to or larger "
                      "than the queue_depth: %d\n",
              binfo.queue_depth);
      exit(1);
    }
  }

  binfo.aiothreads_per_device =
      iniparser_getint(cfg, (char *)"kvs:aiothreads_per_device", 2);
  ...
  str = iniparser_getstring(cfg, (char *)"kvs:emul_configfile",
                            (char *)"/tmp/kvemul.conf");
...
*/
...
#if defined(__AS_BENCH)
  str =
      iniparser_getstring(cfg, (char *)"aerospike:hosts", (char *)"127.0.0.1");
  strcpy(binfo.device_path, str);

  binfo.as_port = iniparser_getint(cfg, (char *)"aerospike:port", 3000);
  binfo.loop_capacity =
      iniparser_getint(cfg, (char *)"aerospike:loop_capacity", 10);
  ...
  binfo.spdk_cache_size =
      iniparser_getint(cfg, (char *)"blobfs:spdk_cache_size", 4096);
  ...
  str =
      iniparser_getstring(cfg, (char *)"db_config:fdb_type", (char *)"hb+trie");
  if (str[0] == 'h' || str[0] == 'H') {
  ...
  // compression
  str = iniparser_getstring(cfg, (char *)"db_config:compression",
                            (char *)"false");
  ...
  binfo.split_pct = iniparser_getint(cfg, (char *)"db_config:split_pct", 100);
  binfo.leaf_pg_size =
  ...
  if (dirname_ret) {
    ...
    */
    ...
    sprintf(binfo.filename, "%s/%s_%s", dbname_postfix, str, dbname_postfix);
  }

  printf("db name %s\n", binfo.filename);

  str =
      iniparser_getstring(cfg, (char *)"db_file:init_filename", binfo.filename);
  strcpy(binfo.init_filename, str);

  str = iniparser_getstring(cfg, (char *)"population:pre_gen", (char *)"false");
  binfo.pre_gen = (str[0] == 't') ? (1) : (0);

  str =
      iniparser_getstring(cfg, (char *)"population:seq_fill", (char *)"false");
  binfo.seq_fill = (str[0] == 't') ? (1) : (0);

  str =
      iniparser_getstring(cfg, (char *)"population:pop_first", (char *)"true");
  binfo.pop_first = (str[0] == 't') ? (1) : (0);

  binfo.pop_nthreads =
      iniparser_getint(cfg, (char *)"population:nthreads", ncores * 2);
  if (binfo.pop_nthreads < 1)
    binfo.pop_nthreads = ncores * 2;
#if defined __KV_BENCH || defined __KVROCKS_BENCH
  if (binfo.pop_nthreads > 1) {
  ...
  binfo.pop_batchsize =
      iniparser_getint(cfg, (char *)"population:batchsize", 1);

  str = iniparser_getstring(cfg, (char *)"population:periodic_commit",
                            (char *)"no");
  if (str[0] == 'n' /*|| binfo.nthreads == 1*/)
    binfo.pop_commit = 0;
  else
    binfo.pop_commit = 1;
  ...
  str = iniparser_getstring(cfg, (char *)"population:fdb_flush_wal",
                            (char *)"no");
  if (str[0] == 'n' /*|| binfo.nthreads == 1*/)
    binfo.fdb_flush_wal = 0;
  else
    binfo.fdb_flush_wal = 1;
  ...
  if (binfo.kp_unitsize < max_key_len) {
    fprintf(stderr, "WARN: Please set 'key_pool_unit' to be equal to or larger "
                    "than the largest possible key size in your config: %d "
                    "--%d\n",
            max_key_len, binfo.kp_unitsize);
    exit(1);
  }
  ...
  str = iniparser_getstring(cfg, (char *)"threads:disjoint_write",
                            (char *)"false");
  if (str[0] == 't' || str[0] == 'T' || str[0] == 'e' || str[0] == 'E') {
    // enabled
  ...
  if (str[0] == 'n') {
    ...
    str = iniparser_getstring(cfg, (char *)"body_length:value_size",
                              (char *)"4096");
    ...
    str = iniparser_getstring(cfg, (char *)"body_length:value_size_ratio",
                              (char *)"100:0");
    i = 0;
    int total_ratio = 0;
    ;
    ...
    fprintf(stdout,
            "Please set value size (body_length) distribution properly:\n");
    fprintf(stdout, "fixed,ratio,uniform,normal\n");
    exit(1);
  }

  if (binfo.vp_unitsize < max_body_buf) {
    fprintf(stderr, "WARN: Please set 'value_pool_unit' to be equal to or "
                    "larger than the largest possible value size in your "
                    "config: %d\n",
            max_body_buf);
    exit(1);
  }
  if (binfo.vp_unitsize == 0 || binfo.kp_unitsize == 0) {
    fprintf(stderr,
            "WARN: Invalide memory pool size, should be greater than 0\n");
    exit(1);
  }
  ...
  if (str[0] == 'n') {
    ...
    binfo.ibatchsize.a = iniparser_getint(
        cfg, (char *)"operation:iterate_batchsize_median", 1000);
    binfo.ibatchsize.b =
  ...
  */
  str = iniparser_getstring(cfg, (char *)"operation:operation_distribution",
                            (char *)"uniform");
  if (str[0] == 'n') {
  ...
  */
  ...
  str = iniparser_getstring(cfg, (char *)"operation:batch_distribution",
                            (char *)"uniform");
  if (str[0] == 'u') {
    binfo.batch_dist.type = RND_UNIFORM;
    binfo.batch_dist.a = 0;
  ...
  str =
      iniparser_getstring(cfg, (char *)"operation:write_type", (char *)"sync");
  binfo.sync_write = (str[0] == 's') ? (1) : (0);

  binfo.compact_thres =
      iniparser_getint(cfg, (char *)"compaction:threshold", 30);
  binfo.compact_period = iniparser_getint(cfg, (char *)"compaction:period", 60);
  binfo.block_reuse_thres =
...
int main(int argc, char **argv) {
  ...
  struct option long_opt[] = { { "database", no_argument, NULL, 'e' },
  ...
  if (strcmp(binfo.log_filename, "")) {
    ...
    sprintf(insert_latency_filename, "%s/%s-insert.latency.csv", str,
            binfo.dbname);
    sprintf(insert_ops_filename, "%s/%s-insert.ops.csv", str, binfo.dbname);
    sprintf(run_latency_filename, "%s/%s-run.latency.csv", str, binfo.dbname);
    sprintf(run_ops_filename, "%s/%s-run.ops.csv", str, binfo.dbname);
