...
struct _db {
    ...
    shannon::WriteOptions *write_options;
                                         ...
                                         Db **pDb)
{
    ...
    *pDb = (Db*)malloc(sizeof(Db));
    ...
    ppdb->devpath = new std::string("/dev/kvdev0");
    
    ppdb->options = new shannon::Options();
    ppdb->options->create_if_missing = true;
    if (!compression) {
        ppdb->options->compression = shannon::kNoCompression;
    }
   ...
   // ppdb->options->compaction_style = shannon::CompactionStyle(compaction_style);
...
/*
    if (cache_size || bloom_bits_per_key) {
        ...
        if (cache_size) {
			table_options.block_cache = shannon::NewLRUCache(cache_size, 6, false, 0.0);
	  //table_options.block_cache = shannon::NewLRUCache(cache_size);
        }
        if (bloom_bits_per_key) {
			table_options.filter_policy.reset(//shannon::NewBloomFilterPolicy(bloom_bits_per_key));
					    shannon::NewBloomFilterPolicy(bloom_bits_per_key));
        }
	table_options.cache_index_and_filter_blocks = 1;
	table_options.index_type = shannon::BlockBasedTableOptions::kBinarySearch;
	...
	table_options.read_amp_bytes_per_bit = 0;
        ppdb->options->table_factory.reset(
            shannon::NewBlockBasedTableFactory(table_options));
    }
*/
    ppdb->options->max_open_files = 500000;

    shannon::Status status = shannon::DB::Open(
        *ppdb->options, *ppdb->filename,"/dev/kvdev0", &ppdb->db);
    if (!status.ok())
    {
    ...
    ppdb->read_options = new shannon::ReadOptions();
    ppdb->write_options = new shannon::WriteOptions();
    ppdb->write_options->sync = true;
    //ppdb->write_options->disableWAL = true;

    assert(status.ok());
    printf("open db %s db_index:%d\n", filename, ppdb->db->GetIndex());
...
couchstore_error_t couchstore_disable_auto_compaction(Db *db, int cpt)
{
    ...
    delete db->db;
    status = shannon::DB::Open(*db->options, *db->filename, *db->devpath, &db->db);
    return COUCHSTORE_SUCCESS;
}
        ...
        unsigned numdocs, couchstore_save_options options)
{
    ...
    for (i=0;i<numdocs;++i){
      /*
        ...
        wb.Put(shannon::Slice(docs[i]->id.buf, docs[i]->id.size),
               shannon::Slice((char*)buf,
      ...
      */
      ...
      wb.Put(shannon::Slice(docs[i]->id.buf, docs[i]->id.size),
	     shannon::Slice((char*)buf, docs[i]->data.size));
      //infos[i]->db_seq = 0;
    }
    free(buf);
...
couchstore_error_t couchstore_docinfo_by_id(Db *db, const void *id, size_t idlen, DocInfo **pInfo)
{
    ...
    status = db->db->Get(*db->read_options, shannon::Slice((char*)id, idlen),
                         value);
        ...
        couchstore_docinfos_options options, couchstore_changes_callback_fn callback, void *ctx)
{
    ...
    for (i=0;i<numDocs;++i){
        status = db->db->Get(*db->read_options,
                             shannon::Slice(ids[i].buf, ids[i].size), value);
					      ...
					      couchstore_open_options options)
{
  ...
  shannon::Status status;
  status = db->db->Delete(*db->write_options, shannon::Slice((char*)id, idlen));
  if(!status.ok()) {
    // not found??
...
{
    int c_ret = 0;
    shannon::Iterator* rit = db->db->NewIterator(shannon::ReadOptions());
    shannon::Slice startkey = shannon::Slice(startDocID->buf, startDocID->size);
    shannon::Slice keyptr, valueptr;
    DocInfo doc_info;
    Doc doc;
...
LIBCOUCHSTORE_API
couchstore_error_t couchstore_status_db(Db* source, const char* target_filename)
{
	shannon::Slice properity("shannon.cur-size-all-mem-tables");
	std::string value;
	if (source) {
		cout<< source->filename<<endl;
		cout<< source->devpath<<endl;
		cout<< source->db->GetIndex()<<endl;
//		source->db->GetDeviceStatus(properity, &value);
	}
    return COUCHSTORE_SUCCESS;
}
