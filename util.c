...
/*void calculate_little_endian()
{
	int i = 0x12345678;
	char *c = (char *)(&i);
	if ((c[0] == 0x78) && (c[1] == 0x56) &&
		(c[2] == 0x34) && (c[3] == 0x12)) {
...
uint32_t decode_fixed32(char* ptr)
{
	if (little_endian) {
		uint32_t result;
		...
		return ((uint32_t)((unsigned char)(ptr[0])))
				| ((uint32_t)((unsigned char)(ptr[1])))
				| ((uint32_t)((unsigned char)(ptr[2])))
...
void set_slice(slice_t *slice, char* start, size_t n)
{
	slice->data = start;
	slice->size = n;
}
...
size_t put_fixed32(char* dst, uint32_t value) {
...
size_t put_fixed64(char* dst, uint64_t value) {
	char buf[sizeof(value)];
...
size_t put_varint32(char *dst, uint32_t v) {
  char buf[5];
...
size_t put_varint64(char *dst, uint64_t v) {
  char buf[10];
...
static  char* get_varint32_ptr(char* p, char* limit, uint32_t* value)
{
	uint32_t result = 0;
	uint32_t shift, byte;
	...
	for (shift = 0; shift <= 28 && p < limit; shift += 7) {
...
static char* get_varint64_ptr(char* p,char* limit, uint64_t* value)
{
	uint64_t result = 0;
	uint64_t shift, byte;
	for (shift = 0; shift <= 63 && p < limit; shift += 7) {
		byte = *((unsigned char *)(p));
		p++;
		if (byte & 128) {
			// More bytes are present
...
bool get_varint64(slice_t* input, uint64_t* value)
{
	char* p = input->data;
	char* limit = p + input->size;
	char* q = get_varint64_ptr(p, limit, value);
	if (q == NULL) {
		return false;
	} else {
		set_slice(input, q, limit - q);
		return true;
	}
}

bool get_varint32(slice_t* input, uint32_t* value)
{
	char* p = input->data;
	char* limit = p + input->size;
	char* q = get_varint32_ptr(p, limit, value);
	if (q == NULL) {
		return false;
	} else {
		set_slice(input, q, limit - q);
...
const uint32_t crc32c_kbyte_extension_table[256] = {
...
const uint32_t crc32c_kstride_extension_table0[256] = {
...
const uint32_t crc32c_kstride_extension_table1[256] = {
...
const uint32_t crc32c_kstride_extension_table2[256] = {
...
const uint32_t crc32c_kstride_extension_table3[256] = {
...
static uint32_t read_uint32_le(uint8_t* buffer) {
	return decode_fixed32((char *)(buffer));
}

// N must be a power of two
static uint8_t* roundup(uint8_t* pointer, int N)
{
	return (uint8_t*)(
...
#define CRC32C_EXTEND_STEP1                              \
  do {                                     \
    int c = (l & 0xff) ^ *p++;             \
    l = crc32c_kbyte_extension_table[c] ^ (l >> 8); \
  } while (0)

#define CRC32C_EXTEND_STEP4(s)                                        \
  do {                                                                \
    crc##s = read_uint32_le(p + s * 4) ^                              \
             crc32c_kstride_extension_table3[crc##s & 0xff] ^         \
             crc32c_kstride_extension_table2[(crc##s >> 8) & 0xff] ^  \
             crc32c_kstride_extension_table1[(crc##s >> 16) & 0xff] ^ \
             crc32c_kstride_extension_table0[crc##s >> 24];           \
  } while (0)

#define CRC32C_EXTEND_STEP16 \
  do {         \
    CRC32C_EXTEND_STEP4(0);  \
    CRC32C_EXTEND_STEP4(1);  \
    CRC32C_EXTEND_STEP4(2);  \
    CRC32C_EXTEND_STEP4(3);  \
    p += 16;   \
  } while (0)

// Process 4 bytes that were already loaded into a word.
#define CRC32C_EXTEND_STEP4W(w)                              \
  do {                                                       \
    w ^= l;                                                  \
    for (i = 0; i < 4; ++i) {                         \
      w = (w >> 8) ^ crc32c_kbyte_extension_table[w & 0xff]; \
    }                                                        \
    l = w;                                                   \
  } while (0)

uint32_t crc32c_extend(uint32_t crc, char* buf, size_t size)
{
	uint32_t kcrc32xor = (uint32_t)(0xffffffffU);
	...
	if ((e - p) >= 16) {
		crc0 = read_uint32_le(p + 0 * 4) ^ l;
		crc1 = read_uint32_le(p + 1 * 4);
		crc2 = read_uint32_le(p + 2 * 4);
		crc3 = read_uint32_le(p + 3 * 4);
		p += 16;

		while ((e - p) >= 16)
			CRC32C_EXTEND_STEP16;
		...
		l = 0;
		CRC32C_EXTEND_STEP4W(crc0);
		CRC32C_EXTEND_STEP4W(crc1);
		CRC32C_EXTEND_STEP4W(crc2);
		CRC32C_EXTEND_STEP4W(crc3);
	}
...
#undef CRC32C_EXTEND_STEP4W
#undef CRC32C_EXTEND_STEP16
#undef CRC32C_EXTEND_STEP4
#undef CRC32C_EXTEND_STEP1
uint32_t crc32c_value(char* data, size_t n) {
	return crc32c_extend(0, data, n);
}

#define crc32c_kmaskdelta 0xa282ead8ul;
static const uint32_t kMaskDelta = 0xa282ead8ul;
uint32_t crc32c_mask(uint32_t crc) {
//	return ((crc >> 15) | (crc << 17)) + (uint32_t)crc32c_kmaskdelta;
	return ((crc >> 15) | (crc << 17)) + kMaskDelta;
}

uint32_t crc32c_unmask(uint32_t masked_crc) {
//	uint32_t rot = masked_crc - (uint32_t)crc32c_kmaskdelta;
	 uint32_t rot = masked_crc - kMaskDelta;
	return ((rot >> 17) | (rot << 15));
}


struct slice *new_slice(char *str, size_t n)
{
	slice_t *sli = (slice_t *)malloc(sizeof(slice_t));
	sli->data = malloc(n);
	memset(sli->data, 0, n);
	memcpy(sli->data, str, n);
	sli->size = n;
	return sli;
}

uint64_t PackSequenceAndType(uint64_t seq, enum ValueType t) {
  assert(seq <= kMaxSequenceNumber);
  assert(t <= kValueTypeForSeek);
  return (seq << 8) | t;
}

struct internalkey_t *internal_key(char *str, int n)
{
	struct internalkey_t *key = malloc(sizeof(struct internalkey_t));
	key->user_key = new_slice(str, n);
	key->sequence = 0x00;
	key->type = 0x01;
	return key;
}

struct slice *extract_userkey(struct slice *internal_key) {
	return new_slice(internal_key->data, internal_key->size - 8);
}

size_t internal_key_size(struct internalkey_t *key) {
  return key->user_key->size + 8;
}

char *append_internalkey(struct internalkey_t *key) {
	int size = internal_key_size(key);
	char *buf = malloc(size);
	memcpy(buf,key->user_key->data, key->user_key->size);
	put_fixed64(buf + key->user_key->size, PackSequenceAndType(key->sequence,key->type));
	return buf;
}

static char *MakeFileName(char *name, uint64_t number,char* suffix) {
  char buf[100];
  snprintf(buf, sizeof(buf), "%06llu.%s",(unsigned long long)(number),
	       suffix);
  char *new_name = (char *) malloc(strlen(name) + sizeof (buf));
	strcpy(new_name, name);
	strcat(new_name, buf);
  return new_name;
}

char *TableFileName(char *name, uint64_t number) {
  assert(number > 0);
  return MakeFileName(name, number, "ldb");
}  

int write_raw(int fd, const char* p, size_t n) {
	while (n > 0) {
...
void get_filesize(char *filename, size_t *file_size)
{
	struct stat statbuf;
	int ret = 0;
